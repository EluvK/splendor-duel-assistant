...
def test_replay_buffer_sliding_window():
    ...
    assert len(out_batch.reason) == 70
...
def test_generate_rust_neural_mcts_selfplay():
    ...
    assert np.all(batch.reason >= 0) and np.all(batch.reason <= 3)
