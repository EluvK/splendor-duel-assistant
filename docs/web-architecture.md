...
1. **三栏自适应主版图**：
   - **左栏 (弹性 minmax(340px, 1fr))**：双人平铺的玩家状态仪表盘（行动方带有蓝色/粉色高亮呼吸边框，支持自适应双列并排或上下平铺）。
   - **中栏 (670px)**：金字塔珠宝卡展示区（Tier 3、Tier 2、Tier 1 纵向堆叠）及下方居中的王室赞助池。
   - **右栏 (290px)**：5×5 螺旋棋盘、特权卷轴剩余池、布袋剩余计数、对战操作历史日志面板及折叠式快捷操作指南。
2. **通栏横幅与控制设计**：
   - **顶部 Header**：导航 Tab 切换、双方席位类型选择（`先手 P0` 与 `后手 P1`）、一键【🔄 交换先后手】按钮、神经网络状态灯与热重载、控制按钮。
   - **胜利者专区 (`winner-banner`)**：获胜时展开的全宽金色发光通告栏，标明获胜方与获胜条件。
   - **交互指引条 (`turn-guide-banner`)**：当前回合玩家的行动指南与动作确认按钮区（动态匹配当前行动方蓝/粉色标，并提示上步 AI 动作与胜率估值）。
