...
5. **智能诊断监测、全场景早停与自适应参数建议 (Training Advisor - 默认内置开启)**：
   - 每轮实时识别：胜因香农多样性熵 ($H$)、Top-1 确定性程度、连续未晋升轮数与胜率震荡度。
   - **5 大异常场景全自动覆盖与早停**：覆盖停滞僵局（连续 15 轮不晋升）、严重模式塌陷（Top-1 > 97.5% 且多样性归零）、灾难性遗忘（胜率暴跌至 < 20% 且 Loss 异常）、策略循环震荡（持续两极分化互克）与对局死锁拖局（平均回合 > 85 轮长对耗）。
   - **自适应调优建议看板**：触发早停或训练结束时，自动诊断根本病灶并生成下一阶段推荐超参数（如调整推演深度、开局温度、学习率或经验池）以及可直接运行的 CLI 破局命令。
...
### 4.4 自博弈核心三维健康运行指标基准表

自博弈训练期间，`TrainingAdvisor` 会对每轮的 Loss、Top-1 准确率及门禁对抗胜率进行三维动态诊断：

| 核心指标 | 🚨 异常偏低（需警惕/早停） | 🟢 绝对健康运行区间 | 🚨 异常偏高（需警惕/早停） | 指标说明与调优建议 |
| :--- | :--- | :--- | :--- | :--- |
| **Loss** | `< 0.60`<br>（丧失探索，网络陷入回音室） | **`0.90 ~ 1.35`**<br>（前沿棋力健康拟合） | `> 2.00`<br>（梯度爆炸、学习率过大震荡） | 联合损失 $\mathcal{L}_{\text{policy}} + 1.0 \times \mathcal{L}_{\text{value}}$。<br>• 偏低：增大 `--dirichlet-eps` 与 `--temp-steps`；<br>• 偏高：降低 `--lr`。 |
| **Top-1** | `< 70.0%`<br>（网络直觉崩溃、缺乏主见） | **`82.0% ~ 89.0%`**<br>（常规步敏捷，关键步有深算空间） | `> 93.0%`<br>（过拟合、丧失策略改进增量） | 网络先验首选命中 MCTS 最终策略的概率。<br>• 偏高：说明先验过于尖锐压制 MCTS，需降低 `--train-epochs`；<br>• 偏低：增加 `--mcts-sims`。 |
| **候选胜率** | `< 42.0%`<br>（持续多轮大幅负向退步） | **`52.0% ~ 58.0%`**<br>（教科书级平稳迭代进化） | `> 75.0%`<br>（两代突然断层，老模型陷入死锁漏洞） | 候选模型对战上一代主力的严格成对换座胜率。<br>• 断层：核查老模型是否存在走子死锁或先后手失衡；<br>• 负退步：平滑学习率与单轮 Epoch。 |
---
